...
```bash
npm install
npm run sample-audio   # 動作確認用の擬似お手本音源を生成（鍵不要・オフライン。下記参照）
...
```bash
npm run sample-audio          # 未生成のみ生成（--force で上書き）
```
各フレーズの声調から北部方言を簡略化したピッチ輪郭を合成し `public/audio/{id}.mp3` を作ります。
**鍵もネット接続も不要**で、ピッチ重ね表示・スコア・進行管理をすぐ確認できます。
⚠ これは声調の“形”だけを近似した合成音で、**発音のお手本ではありません**。実運用では下記の TTS か自前録音に差し替えてください。
